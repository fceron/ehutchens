...
{title: Ehutchens: Hutchens `square root' segregation index, with decompositions by subgroup, and extended options for supergroups, stored matrices and new datasets}
        ...
        {cmdab:ehutchens} {it:unitvar} {it:segvar} [{it:weight}] [{cmdab:if} exp] [{cmdab:in} range] [, {cmdab:by:group}(groupvar) {cmdab:m:issing} {cmdab:f:ormat}(%fmt) {cmdab:su:pergroup}(varname) {cmdab:s:ave}(string) {cmdab:c:lear}] [{cmdab:b:ootstrap} {cmdab:re:ps}(integer 400) {cmdab:se:ed}(integer 12345) {cmdab:cl:uster}(varname)]
...
{title: Options}
	...
	{cmdab:bootstrap} {cmdab:reps}(integer 400){cmdab:seed}(integer 12345){cmdab:cluster}({it:varname}) sets the number of bootstrap samples in {cmdab:reps} (400 by default), sets the random-number seed in {cmdab:seed} (12345 by default) and {cmdab:cluster} specifies the social units 
	defined in {it:unitvar} as resampling clusters. This option is defined from the built-in bootstrap Stata command, allowing sample weights and storing estimations, but its usage is restricted to the three suboptions already 
	mentioned. It runs hu_int, the original hutchens command slightly modified, as an internal program, for each bootstrap replication after the supergroup calculations are completed. Estimations are stored in a matrix. 
	This option cannot be used with save option.
...
{title: Examples} 
    ...
    Sex segregation in schools, with a decomposition by school type across regions (supergroup option) and save results as new dataset:
	
	. ehutchens schoolid sex, by(stype) supergroup(region) save(mydata.dta)
	
    Sex segregation in schools, with a decomposition by school type across regions (supergroup option) and replacing current dataset by results dataset:
	
	. ehutchens schoolid sex, by(stype) supergroup(region) save(mydata.dta) clear

    Sex segregation in schools, with a decomposition by school type across regions (supergroup option) and bootstrap of main results:
...
Author (***revising***)
	...
	Francisco I. Ceron, LM2C2, Pontificia Universidad Católica de Chile / Amsterdam Institute for Social Science Research (AISSR), University of Amsterdam, The Netherlands. Email: fcerona@uc.cl 
