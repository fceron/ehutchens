...
{title: Options}
	...
	{cmdab:bootstrap} {cmdab:reps}(integer 400){cmdab:seed}(integer 12345){cmdab:cluster}({it:varname}) sets the number of bootstrap samples in {cmdab:reps} (400 by default), sets the random-number seed in {cmdab:seed} (12345 by default) and {cmdab:cluster} 
	specifies the social units defined in {it:unitvar} as resampling clusters. This option is defined from the built-in bootstrap Stata command, allowing sample weights and storing estimations, but its usage is restricted to the 
	three suboptions already mentioned. It runs the original hutchens command slightly modified, as an internal program, for each bootstrap replication after the supergroup calculations are completed. 
